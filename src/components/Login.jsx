...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setStatus("Entrando..."); setStatusType(""); setLoading(true);
    ...
    if (error) {
      ...
      setStatus(msg); setStatusType("error"); setLoading(false); return;
    }
    ...
    if (!perfil) {
      ...
      setStatus("Usuário não cadastrado no sistema."); setStatusType("error");
      setLoading(false); return;
    }

    setStatus("Login efetuado!"); setStatusType("success");
    navigate("/");
  };
      ...
      <div className="bg-white/10 backdrop-blur-md rounded-2xl shadow-2xl max-w-md w-full p-8 border border-white/20">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            ...
            className="w-full bg-white/20 text-white py-3 rounded-lg hover:bg-white/30 transition font-medium disabled:opacity-50"
          >
        ...
        {status && (
          <p className={`mt-4 text-sm text-center ${statusType === "success" ? "text-green-400" : "text-red-400"}`}>
            {status}
          </p>
        )}
      </div>
